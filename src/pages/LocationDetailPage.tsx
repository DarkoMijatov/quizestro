...
import {
  MapPin, Clock, Calendar, Globe, ExternalLink, Instagram, Facebook,
  Mail, Phone, Navigation, Loader2, ArrowLeft, Users, Trophy, DollarSign,
} from 'lucide-react';
    ...
    <ForceDarkTheme>
      <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 pt-24 pb-16">
          <Link to="/#quiz-map" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-primary mb-6">
          ...
          {loading ? (
            ...
            <div className="grid lg:grid-cols-5 gap-8">
                        ...
                        <div key={s.id} className="rounded-xl border border-border bg-card p-4 space-y-2">
                          <div className="flex items-center gap-2 flex-wrap">
                            {s.schedule_type === 'recurring' ? (
                              <Badge className="bg-primary/10 text-primary border-primary/30">{t('map.recurringSchedule')}</Badge>
                            ) : (
                          ...
                          <div className="flex items-center gap-2 text-sm">
                            {s.schedule_type === 'recurring' ? <Clock className="h-4 w-4 text-primary" /> : <Calendar className="h-4 w-4 text-primary" />}
                            <span>{getScheduleText(s)}</span>
                            {s.end_time && <span className="text-muted-foreground">– {s.end_time.slice(0, 5)}</span>}
                          </div>
                          <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                            {s.entry_fee && <span className="flex items-center gap-1"><DollarSign className="h-3 w-3" /> {s.entry_fee}</span>}
                            {s.prize_info && <span className="flex items-center gap-1"><Trophy className="h-3 w-3" /> {s.prize_info}</span>}
                            {s.team_size_info && <span className="flex items-center gap-1"><Users className="h-3 w-3" /> {s.team_size_info}</span>}
                            {s.language && <span className="flex items-center gap-1"><Globe className="h-3 w-3" /> {s.language}</span>}
                          </div>
                  ...
                  {(location.contact_email || location.contact_phone) && (
                    <div className="flex flex-wrap gap-4">
                      {location.contact_email && (
                        <a href={`mailto:${location.contact_email}`} className="inline-flex items-center gap-1.5 text-sm text-primary hover:underline">
                          <Mail className="h-4 w-4" /> {location.contact_email}
                        </a>
                      )}
                      {location.contact_phone && (
                        <a href={`tel:${location.contact_phone}`} className="inline-flex items-center gap-1.5 text-sm text-primary hover:underline">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {location.website_url && (
                      <a href={location.website_url} target="_blank" rel="noopener noreferrer">
                        <Button variant="outline" size="sm" className="gap-1.5"><Globe className="h-3.5 w-3.5" />{t('map.website')}</Button>
                      </a>
                    )}
                    {location.reservation_url && (
                      <a href={location.reservation_url} target="_blank" rel="noopener noreferrer">
                        <Button variant="outline" size="sm" className="gap-1.5"><ExternalLink className="h-3.5 w-3.5" />{t('map.reservation')}</Button>
                      </a>
                    )}
                    ...
                    {location.facebook_url && (
                      <a href={location.facebook_url} target="_blank" rel="noopener noreferrer">
                        <Button variant="outline" size="sm" className="gap-1.5"><Facebook className="h-3.5 w-3.5" />Facebook</Button>
                      </a>
                    )}
                    {location.latitude && location.longitude && (
                      ...
                      >
                        <Button variant="outline" size="sm" className="gap-1.5"><Navigation className="h-3.5 w-3.5" />{t('map.openInMaps')}</Button>
                      </a>
                    )}
                  </div>
                </div>
              ...
              <div className="lg:col-span-2">
                {location.latitude && location.longitude ? (
                  <div className="rounded-xl overflow-hidden border border-border sticky top-24" style={{ height: 350 }}>
                    <MapContainer
