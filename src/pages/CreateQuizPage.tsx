...
import {
  CalendarIcon, ArrowLeft, ArrowRight, Check, Loader2, Trophy, FolderOpen, Users,
} from 'lucide-react';
...
export default function CreateQuizPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoadingData(true);
      const [catRes, teamRes, leagueRes] = await Promise.all([
  ...
  const canNext = () => {
    switch (step) {
      case 0: return quizName.trim().length > 0;
      case 1: return selectedCats.length > 0;
      case 2: return selectedTeams.length >= 2;
      case 3: return true;
    ...
    <DashboardLayout>
      <div className="max-w-2xl mx-auto space-y-8">
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <Checkbox
                        ...
                        onCheckedChange={() => setSelectedLeague(null)}
                      />
        ...
        <div className="flex justify-between">
          <Button variant="outline" onClick={() => step > 0 ? setStep(step - 1) : navigate('/dashboard/quizzes')} className="gap-1">
            <ArrowLeft className="h-4 w-4" />
            {t('common.back')}
          </Button>
